...
def non_adult_horror_movies(title_df, ratings_df):
    ...
    non_adult_movies.select("primaryTitle", "genres", "averageRating").orderBy(
        col("averageRating").desc()
    ).show(20, truncate=False)
    non_adult_movies.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.non_adult_horrors"
    )


def top_rated_movies(ratings_df, title_df):
    top_movies = ratings_df.join(title_df, on="tconst").filter(
        (col("averageRating") > 8.0) & (col("numVotes") > 10000)
    )
    top_movies.select("primaryTitle", "averageRating", "numVotes").orderBy(
        col("averageRating").desc()
    ).show(20, truncate=False)
    top_movies.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.top_rated_movies"
    )
...
def movies_per_year(title_df):
    yearly_movies = (
        title_df.filter(
            (col("startYear").between(2000, 2024)) & (col("titleType") == "movie")
        )
        .groupBy("startYear")
        .agg(count("*").alias("numMovies"))
    )

    yearly_movies.orderBy(col("startYear").desc()).show(20, truncate=False)
    yearly_movies.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.movies_per_year"
    )
...
def actor_average_rating(principals_df, ratings_df, names_df, title_df):
    ...
    actor_rating_avg = actor_filtered.withColumn(
        "average_rating", round(col("weighted_sum") / col("total_votes"), 2)
    )

    actor_with_names = actor_rating_avg.join(
        names_df.select("nconst", "primaryName"), on="nconst"
    )
    ...
    ranked.select(
        "rank", "primaryName", "num_movies", "average_rating", "total_votes"
    ).orderBy("rank").show(20, truncate=False)
    ranked.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.actor_average_rating"
    )


def longest_movies_writers(title_df, crew_df, names_df, top_n=20):
    movies = title_df.filter(
        (col("titleType") == "movie") & (col("runtimeMinutes").isNotNull())
    ).withColumn("runtimeMinutes", col("runtimeMinutes"))

    movies_with_writers = movies.join(crew_df, on="tconst").filter(
        col("writers").isNotNull() & (col("writers") != "\\N")
    )

    window_spec = Window.orderBy(col("runtimeMinutes").desc())
    top_movies = movies_with_writers.withColumn(
        "rank", row_number().over(window_spec)
    ).filter(col("rank") <= top_n)

    longest_writers = top_movies.withColumn(
        "writer_id", explode(split(col("writers"), ","))
    )
    ...
    writers_with_names.select("primaryTitle", "runtimeMinutes", "primaryName").orderBy(
        col("runtimeMinutes").desc(), col("primaryTitle")
    ).show(20, truncate=False)
    writers_with_names.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.writers_of_longest_movies"
    )


def underrated_directors(
    title_df,
    crew_df,
    ratings_df,
    names_df,
    ...
    with_names = director_stats.join(
        names_df.select("nconst", "primaryName"),
    ...
    with_rank = with_names.withColumn("rank", row_number().over(window_spec)).filter(
        col("rank") <= top_n
    )

    with_rank.select("rank", "primaryName", "avg_rating", "num_movies").orderBy(
        "rank"
    ).show(20, truncate=False)
    with_rank.write.mode("overwrite").option("header", "true").csv(
        "output/pavlo.underrated_directors"
    )


def call_pavlo_functions(title_df, ratings_df, names_df, principals_df, crew_df):
    ...
    underrated_directors(
        title_df, crew_df, ratings_df, names_df, min_votes=1000, max_movies=5
    )
