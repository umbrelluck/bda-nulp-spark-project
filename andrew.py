...
def drama_actors(title_basics_df, title_principals_df, name_basics_df):
    ...
    q1 = long_drama_df.join(actor_roles_df, "tconst") \
        .groupBy("nconst") \
...
def films_after_2010(title_basics_df, title_ratings_df):
    ...
    q2.show(20)
    q2.write.mode("overwrite").option("header", True).csv("output/andrew.films_after_2010")

def documentary_films_1990_to_2024(title_basics_df):
    q3 = title_basics_df.filter(
        (F.col("genres").contains("Documentary")) &
        (F.col("startYear").between(1990, 2024)) &
        (F.col("titleType") == "movie")
    ).groupBy("startYear").agg(F.count("*").alias("documentary_count")) \
        .orderBy("startYear")
    q3.show(20)
    q3.write.mode("overwrite").option("header", True).csv("output/andrew.documentary_films_1990_to_2024")

def tv_series_with_most_seasons(title_basics_df, title_episode_df, title_crew_df, name_basics_df):
    tv_series_df = title_basics_df.filter(F.col("titleType") == "tvSeries") \
        .select("tconst", "primaryTitle")

    episodes_per_series = title_episode_df.groupBy("parentTconst") \
        .agg(F.count("*").alias("episode_count"))

    crew_df = title_crew_df.select("tconst", "directors") \
        .withColumn("director", explode(split("directors", ",")))


    directors_df = crew_df.join(name_basics_df.select("nconst", "primaryName"),
    ...
    q4 = episodes_per_series \
        .join(tv_series_df, episodes_per_series["parentTconst"] == tv_series_df["tconst"]) \
    ...
    q4.write.mode("overwrite").option("header", True).csv("output/andrew.tv_series_with_most_seasons")

def actors_under_25(title_principals_df, name_basics_df, title_ratings_df, title_basics_df):
    ...
    debut_joined = actors_df.join(title_basics_df, "tconst") \
    ...
    q5 = debut_joined.withColumn("rn", F.row_number().over(window_spec)) \
    ...
    q5.write.mode("overwrite").option("header", True).csv("output/andrew.actors_under_25")

def top_3_in_each_category(title_basics_df, title_ratings_df):
    rated_movies_df = title_basics_df.filter(F.col("titleType") == "movie") \
        .join(title_ratings_df, "tconst") \
        .withColumn("genre", explode(split(F.col("genres"), ",")))
    genre_window = Window.partitionBy("genre").orderBy(F.desc("numVotes"))
    q6 = rated_movies_df.withColumn("rank", F.row_number().over(genre_window)) \
        .filter(F.col("rank") <= 3) \
        .select("primaryTitle", "genre", "numVotes", "rank")
    q6.show(20)
    q6.write.mode("overwrite").option("header", True).csv("output/andrew.top_3_in_each_category")


def call_andrew_functions(
        title_basics_df, title_principals_df, title_ratings_df, title_crew_df, name_basics_df,
        title_episode_df
):
    print("Actors which most frequently appear in movies that are over 150 minutes long and belong to the Drama genre")
    drama_actors(title_basics_df, title_principals_df, name_basics_df)

    print("Movies released after 2010 have a rating below 5 and received more than 5,000 votes")
    ...
    tv_series_with_most_seasons(title_basics_df, title_episode_df, title_crew_df, name_basics_df)

    print("Actors that made their debut in movies before the age of 25 and had the highest ratings for their debut roles")
    actors_under_25(title_principals_df, name_basics_df, title_ratings_df, title_basics_df)
